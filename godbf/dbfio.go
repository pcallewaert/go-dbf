...
func NewFromFileRead(fileName string, fileEncoding string) (table *DbfTable, err error) {
	f, err := os.Open(fileName)
	if err != nil {
		return nil, err
	}
	s, err := mmap.Map(f, mmap.RDONLY, 0)
	if err != nil {
		return nil, err
	}
...
func (dt *DbfTable) Close() (err error) {
	if dt.mmapHandler != nil {
		err = dt.mmapHandler.Unmap()
		if err != nil {
			return err
		}
	}
	if dt.fileHandler != nil {
		err = dt.fileHandler.Close()
		if err != nil {
			return err
		}
	}
